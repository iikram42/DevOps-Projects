...
**GITHUB REPO**: [Chatbot-UI](https://github.com/iikram42/DevOps-Projects.gitI)

```
 cd DevOps-Projects/DevOps-Project-28/Chatbot-UI/Jenkins-Server-TF
...
pipeline {
    ...
    stages {
            ...
            steps {
                script {
                    withDockerRegistry(credentialsId: 'docker', toolName: 'docker') {
                        ...
                        sh "docker tag chatbot iikram42/chatbot:latest"
                        sh "docker push iikram42/chatbot:latest"
                    }
                }
            }
        }
        stage("TRIVY Image SCAN") {
            steps {
                sh "trivy image iikram42/chatbot:latest > trivy.json"
            }
        }
        ...
        stage('Deploy to Local Docker') {
            steps {
                sh 'docker run -d --name chatbot -p 3000:3000 iikram42/chatbot:latest'
            }
        }
        stage('Deploy to Kubernetes') {
            steps {
